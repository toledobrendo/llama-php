<?php
  ...
  function saveOrder($tireQty, $oilQty, $sparkQty, $totalAmount) {
    ...
    $file = @ fopen(DOCUMENT_ROOT.'/llama-php/hw4/resource/orders.txt', 'ab');
  ...
  function getOrders() {
    try {
      $file = @ fopen(DOCUMENT_ROOT.'/llama-php/hw4/resource/orders.txt', 'rb');
  ...
  function getVAT_PERCENT() {
    try {
      $file = @fopen(DOCUMENT_ROOT.'/llama-php/hw4/resource/properties.txt', 'rb');
